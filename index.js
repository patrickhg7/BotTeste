...
fs.readdir("./commands/", (err, files) => {
  ...
  files.forEach(file => {
    ...
    console.log(`carregando o comando: t!${commandName}`);
...
client.on('raw', payload => {
  if(payload.t === 'MESSAGE_REACTION_ADD') { // Check if the event name is MESSAGE_REACTION_ADD
      ...
      {
          if(payload.d.message_id === '755278204075507732') { // Here we check if the id of the message is the ID of the embed that we had the bot send using the ?sendmsg command.
...
client.on('messageReactionAdd', (reaction, user) => {
  if(reaction.emoji.name === '👾') { // If the emoji name is ticketreact, we will create the ticket channel.
       ...
       */
      if(userTickets.has(user.id) || reaction.message.guild.channels.cache.find(channel => channel.name.toLowerCase() === user.username + 's-ticket')) {
          ...
          guild.createChannel(`${user.username}s-ticket`, {
              ...
              let embed = new Discord.DiscordEmbed();
      ...
      else if(reaction.message.guild.channels.cache.find(channel => channel.name.toLowerCase() === user.username + 's-ticket')) {
          let embed = new Discord.DiscordEmbed();
          embed.setDescription("Ticket will be closed in 5 seconds.");
          reaction.message.channel.send(embed);
          setTimeout(() => {
